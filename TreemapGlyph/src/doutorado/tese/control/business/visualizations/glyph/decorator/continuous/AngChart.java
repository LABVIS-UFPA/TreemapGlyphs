 ...
 * @author LABVIS
 */
public class AngChart extends Glyph {
...
//    private Point center;
    ...
    public AngChart(List<String> variaveisEscolhidasStarGlyph) {
...
//                    (int) (getSlices()[i].getDado()*360 /getSlices()[i].getDadoMaxVal());
            ...
            float result = (data*w)/max;
            float innerX = w-result;
            float innerY = w-result;
            
            getSlices()[i].setDadosBarra(Math.round(positionX+innerX/2),Math.round(positionY+innerY/2),Math.round(result),Math.round(result), startAngle,arcPorcent);
            curValue += 90;
